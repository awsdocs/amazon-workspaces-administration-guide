...
+ To allow access from PCoIP zero client devices, you must be using a PCoIP protocol bundle for WorkSpaces\. You must also enable Network Time Protocol \(NTP\) in Teradici\. For more information, see [Set up PCoIP zero clients for WorkSpaces](set-up-pcoip-zero-client.md)\.
